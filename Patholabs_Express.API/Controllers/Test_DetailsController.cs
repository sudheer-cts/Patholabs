...
namespace Patholabs_Express.API.Controllers
{
    public class Test_DetailsController : ApiController
    {
        ...
        [HttpGet]
        [Route("api/Test_Details/SearchResult/{SearchTest}")]
        public IHttpActionResult SearchResult([FromUri] string SearchTest)

        {
            var item = testService.getallSearchResult(SearchTest);
            if (item!=null && item.Count()!=0)
            {
                return Ok(new Responce() { Success = true, Message = "Price updated successfully", Result=item });
            }
            else
                return BadRequest();
        }
