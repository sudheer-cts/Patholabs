...
namespace Patholabs_Express.BuisnessLogic.Services
{
        ...
        public List<Test_Details> getallSearchResult(string SearchTest)
        {
            List<Test_Details> all_test = getall();
            List<Test_Details> search_result = new List<Test_Details>();
            var result = all_test.Where(T => T.TestName!=null && T.TestName.Contains(SearchTest));
            if (result != null)
            {
                search_result = result.ToList();
            }
            else
            {
                search_result = null;
